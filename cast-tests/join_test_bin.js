...
castv2testing.e2e.Controller.prototype.removeMessageListener = function(namespace, listener) {
  this.session_ ? (this.logger_.info("Removing message listener."), this.session_.removeMessageListener(namespace, listener)) : this.logger_.info("No session available");
};
...
function removeMessageListener() {
  controller.removeMessageListener("urn:x-cast:com.google.cast.castv2debug.broadcastecho", onReceiverMessage_);
  console.log("remove message listener");
}
function sendUnicastMessage(message) {
  isSuccessful = !1;
  controller.sendMessage("urn:x-cast:com.google.cast.castv2debug.echo", message, function() {
    isSuccessful = !0;
  }, function() {
    console.error("fail to send unicast message");
  });
}
function sendBroadcastMessage(msg) {
  isSuccessful = !1;
  message = null;
  controller.sendMessage("urn:x-cast:com.google.cast.castv2debug.broadcastecho", msg, function() {
    isSuccessful = !0;
  }, function() {
    console.error("fail to send broadcast message");
  });
}
function onReceiverMessage_(namespace, msg) {
  console.log("got message: " + namespace + " " + msg);
  message = msg;
}
;goog.json = {};
