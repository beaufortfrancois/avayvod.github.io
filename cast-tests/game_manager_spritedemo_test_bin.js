...
var MESSAGE = {type:1}, controller = null, gameManagerController = null, gameManagerTestHelper = null;
function setUpPage() {
...
function testGameMessage() {
  ...
  asyncTestCase.waitForSignals(1, "Waiting for game message to be sent");
}
function testValidPlayerTransitions() {
  var currentPlayer = gameManagerTestHelper.assertStateAndGetPlayer("Player1");
  assertEquals(currentPlayer.getPlayerState(), chrome.cast.games.PlayerState.PLAYING);
  var playing = chrome.cast.games.PlayerState.PLAYING, idle = chrome.cast.games.PlayerState.IDLE;
  gameManagerTestHelper.asyncAssertAndSignalPlayerStateTransitions("Player1", [chrome.cast.games.PlayerState.READY, playing, idle, playing, idle, playing, idle, playing, idle]);
  asyncTestCase.waitForSignals(1, "Waiting for transition tests to complete");
}
