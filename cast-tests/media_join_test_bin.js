...
castv2testing.e2e.Controller.prototype.joinSession = function(sessionId) {
  this.logger_.info("Join session");
  chrome.cast.requestSessionById(sessionId);
};
castv2testing.e2e.Controller.prototype.leaveSession = function(successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("Leaving " + this.session_.sessionId), this.session_.leave(this.onSuccess_.bind(this, successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("No session available"), errorCallback && errorCallback());
};
...
castv2testing.e2e.Controller.prototype.addSessionListener = function(listener) {
  this.session_ ? (this.logger_.info("Adding session listener."), this.session_.addUpdateListener(listener)) : this.logger_.info("No session available");
};
castv2testing.e2e.Controller.prototype.removeSessionListener = function(listener) {
  this.session_ ? (this.logger_.info("Removing session listener."), this.session_.removeUpdateListener(listener)) : this.logger_.info("No session available");
};
castv2testing.e2e.Controller.prototype.addMediaListener = function(listener) {
  this.session_ ? (this.logger_.info("Adding media listener."), this.session_.addMediaListener(this.onSessionMediaUpdate_.bind(this, listener))) : this.logger_.info("No session available");
};
castv2testing.e2e.Controller.prototype.removeMediaListener = function(listener) {
  this.session_ ? (this.logger_.info("Removing media listener."), this.session_.removeMediaListener(listener)) : this.logger_.info("No session available");
};
...
castv2testing.e2e.Controller.prototype.onSessionMediaUpdate_ = function(callback, media) {
  this.logger_.info("Calling session media listener");
  this.logger_.info("media: " + JSON.stringify(media));
  this.media_ = media;
  callback && callback(media);
};
...
castv2testing.e2e.JoinUtils = {};
var controller = new castv2testing.e2e.Controller, isSuccessful = !1, isLive = !1, currentSession = null, initSuccessful = !1, isReceiverAvailable = !1;
function loadScript() {
  isSuccessful = !1;
  chrome.cast && chrome.cast.isAvailable ? isSuccessful = !0 : setTimeout(loadScript, 1000);
}
function init(appId, opt_autoJoinPolicy) {
  isSuccessful = !1;
  controller.initialize(appId, sessionListener_, function(availability) {
    console.log("receiver is " + availability);
    initSuccessful && !isReceiverAvailable && availability == chrome.cast.ReceiverAvailability.AVAILABLE && (isSuccessful = isReceiverAvailable = !0);
  }, function() {
    initSuccessful = !0;
    console.log("initialization successfully");
  }, function(error) {
    console.error("fail to initialize API");
  }, opt_autoJoinPolicy);
}
...
function joinSession(session_id) {
  controller.joinSession(session_id);
}
function _requestSession() {
  controller.requestSession(function(session) {
    session && session.sessionId && (isLive = isSuccessful = !0, console.log("request session successfully"));
  }, function(error) {
    console.error("fail to request session");
  });
}
function addSessionUpdateListener() {
  controller.addSessionListener(onSessionUpdate_);
  console.log("add session update listenser");
}
function removeSessionUpdateListener() {
  controller.removeSessionListener(onSessionUpdate_);
  console.log("remove session update listener");
}
function leaveSession() {
  isSuccessful = !1;
  controller.leaveSession(function(session) {
    isSuccessful = !0;
    console.log("leave session successfully");
  }, function(error) {
    console.error("fail to leave session");
  });
}
function stopSession() {
  ...
  controller.stopSession(function(session) {
    isSuccessful = !0;
    console.log("stop session successfully");
  }, function(error) {
    console.error("fail to stop session");
  });
}
function getSession() {
  return controller.session_;
}
function onSessionUpdate_(live) {
  console.log("session updated");
  isLive = live;
}
function sessionListener_(session) {
  currentSession = session;
}
;var mediaIsAlive = !1, mediaListener = !1, sessionMediaListener = !1;
function initialization() {
  init("BE6E4473", chrome.cast.AutoJoinPolicy.ORIGIN_SCOPED);
}
function loadMedia(mediaUrl) {
  ...
  controller.loadMedia(mediaUrl || "http://avayvod.github.io/cast-tests/video.webm", !0, function(media) {
    ...
    console.error("Fail to load media");
  });
}
function pauseMedia() {
  isSuccessful = !1;
  controller.pauseMedia(function() {
    isSuccessful = !0;
    console.log("Pause media successfully");
  }, function(error) {
    console.error("Fail to pause media");
  });
}
function playMedia() {
  isSuccessful = !1;
  controller.playMedia(function() {
    isSuccessful = !0;
    console.log("Play media successfully");
  }, function(error) {
    console.error("Fail to play media");
  });
}
function getMedia() {
  return controller.media_;
}
function addMediaUpdateListener() {
  controller.addMediaUpdateListener(onMediaUpdate_);
  console.log("add media update listener");
}
function removeMediaUpdateListener() {
  controller.removeMediaUpdateListener(onMediaUpdate_);
  console.log("remove media update listener");
}
function addMediaListener() {
  controller.addMediaListener(onSessionMediaUpdate_);
  console.log("add session media listener");
}
function removeMediaListener() {
  controller.removeMediaListener(onSessionMediaUpdate_);
  console.log("remove session media listener");
}
function onMediaUpdate_(isAlive) {
  console.log("got media update");
  mediaIsAlive = isAlive;
  mediaListener = !0;
}
function onSessionMediaUpdate_(media) {
  console.log("got session media update");
  sessionMediaListener = !0;
}
;goog.json = {};
