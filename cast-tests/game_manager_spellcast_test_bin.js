...
castv2testing.e2e.GameManagerController.prototype.listenForGameDataChange = function(gameData, callback) {
  var state = this.getState();
  if (!goog.testing.asserts.findDifferences(gameData, state.getGameData())) {
    return callback(), !0;
  }
  var listener = goog.bind(function(event) {
    var currentState = event.currentState;
    currentState.hasGameDataChanged(event.previousState) && !goog.testing.asserts.findDifferences(gameData, currentState.getGameData()) && (this.gameManagerClient_.removeEventListener(chrome.cast.games.GameManagerEventType.STATE_CHANGED, listener), callback());
  }, this);
  this.gameManagerClient_.addEventListener(chrome.cast.games.GameManagerEventType.STATE_CHANGED, listener);
  return !1;
};
...
castv2testing.e2e.GameManagerTestHelper.prototype.asyncAssertAndSignalGameDataChanged = function(gameData) {
  var callback = function() {
    var currentGameData = this.gameManagerController_.getState().getGameData();
    assertObjectEquals(currentGameData, gameData);
    this.asyncTestCase_.signal();
  }.bind(this);
  this.gameManagerController_.listenForGameDataChange(gameData, callback) && this.asyncTestCase_.signal();
};
...
castv2testing.e2e.GameManagerTestHelper.prototype.asyncAssertAndSignalChangePlayerStateWithData = function(playerId, playerState, messageData, playerData, gameplayState, lobbyState) {
  var successCallback = function(result) {
    var currentState = this.gameManagerController_.getState();
    ...
    assertEquals(result.playerId, playerId);
    this.assertPlayerIsConnectedControllable(playerId, playerState);
    assertObjectEquals(currentState.getPlayer(playerId).getPlayerData(), playerData);
    this.asyncTestCase_.signal();
  }.bind(this);
  (0,this.stateChangeFunctions_[playerState])(playerId, messageData, successCallback, function(error) {
    fail("Failed to transition with data : " + JSON.stringify(error));
  });
};
...
castv2testing.e2e.GameManagerTestHelper.prototype.asyncAssertAndSignalKillPlayer = function(playerId) {
  var successCallback = function(result) {
    assertEquals(this.gameManagerController_.getLastUsedPlayer(), playerId);
    assertEquals(result.playerId, playerId);
    var currentState = this.gameManagerController_.getState(), currentPlayers = currentState.getPlayers(), currentPlayer = this.findPlayer(playerId, currentPlayers);
    assertEquals(currentPlayer.getPlayerId(), playerId);
    assertEquals(currentPlayer.getPlayerState(), chrome.cast.games.PlayerState.QUIT);
    assertTrue(currentPlayer.isControllable());
    var controllablePlayers = currentState.getControllablePlayers(), connectedPlayers = currentState.getConnectedPlayers(), connectedControllablePlayers = currentState.getConnectedControllablePlayers(), quitPlayers = currentState.getPlayersInState(chrome.cast.games.PlayerState.QUIT);
    assertNotNullNorUndefined(this.findPlayer(playerId, controllablePlayers));
    assertNotNullNorUndefined(this.findPlayer(playerId, quitPlayers));
    assertNull(this.findPlayer(playerId, connectedControllablePlayers));
    assertNull(this.findPlayer(playerId, connectedPlayers));
    this.asyncTestCase_.signal();
  }.bind(this), errorCallback;
  this.gameManagerController_.changePlayerStateToQuit(playerId, null, successCallback, function() {
    fail("Failed to Kill Player.");
  });
};
...
var READY_DATA = {playerName:"TestPlayerName", avatarIndex:0}, PLAYER_DATA = {playerName:"TestPlayerName", avatarIndex:0, host:!0}, PLAYING_DATA = {difficultySetting:1}, WAITING_FOR_PLAYERS_GAME_DATA = {gameStateId:1}, INSTRUCTIONS_GAME_DATA = {gameStateId:2}, PLAYER_ACTION_GAME_DATA = {gameStateId:3}, PLAYER_RESOLUTION_GAME_DATA = {gameStateId:4}, ENEMY_RESOLUTION_GAME_DATA = {gameStateId:5}, SPELL_MESSAGE = {spells:[{spellType:1, spellElement:2, spellAccuracy:3}]}, controller = null, gameManagerController = 
null, gameManagerTestHelper = null;
function setUpPage() {
  ...
  asyncTestCase.setTests([new goog.testing.TestCase.Test("testSetupSession", testSetupSession), new goog.testing.TestCase.Test("testSetupGameManagerClient", testSetupGameManagerClient), new goog.testing.TestCase.Test("testWaitForLobbyScreen", testWaitForLobbyScreen), new goog.testing.TestCase.Test("testCreatePlayer", testCreatePlayer), new goog.testing.TestCase.Test("testReadyPlayer", testReadyPlayer), new goog.testing.TestCase.Test("testStartPlaying", testStartPlaying), new goog.testing.TestCase.Test("testShowInstructions", 
  testShowInstructions), new goog.testing.TestCase.Test("testShowGameScreen", testShowGameScreen), new goog.testing.TestCase.Test("testProcessPlayerAndEnemyMove", testProcessPlayerAndEnemyMove), new goog.testing.TestCase.Test("testKillPlayer", testKillPlayer), new goog.testing.TestCase.Test("testTeardown", testTeardown)]);
}
function testSetupSession() {
  gameManagerTestHelper.asyncAssertAndSignalLoadAPIScript();
  gameManagerTestHelper.asyncAssertAndSignalInitializeController("E92ACE28");
...
function testSetupGameManagerClient() {
  gameManagerTestHelper.asyncAssertAndSignalCreateGameManagerClient([chrome.cast.games.GameplayState.LOADING, chrome.cast.games.GameplayState.RUNNING, chrome.cast.games.GameplayState.SHOWING_INFO_SCREEN]);
  asyncTestCase.waitForSignals(1, "Waiting to create game manager client");
}
function testWaitForLobbyScreen() {
  gameManagerTestHelper.asyncAssertAndSignalGameplayStateChanged(chrome.cast.games.GameplayState.SHOWING_INFO_SCREEN);
  gameManagerTestHelper.asyncAssertAndSignalGameDataChanged(WAITING_FOR_PLAYERS_GAME_DATA);
  asyncTestCase.waitForSignals(2, "Waiting for lobby screen");
}
function testCreatePlayer() {
  gameManagerTestHelper.asyncAssertAndSignalCreatePlayer("Player1", chrome.cast.games.PlayerState.AVAILABLE, 1);
  asyncTestCase.waitForSignals(1, "Waiting for player to be created");
}
function testReadyPlayer() {
  gameManagerTestHelper.asyncAssertAndSignalChangePlayerStateWithData("Player1", chrome.cast.games.PlayerState.READY, READY_DATA, PLAYER_DATA, chrome.cast.games.GameplayState.SHOWING_INFO_SCREEN, chrome.cast.games.LobbyState.OPEN);
  asyncTestCase.waitForSignals(1, "Waiting for player to be ready to play");
}
function testStartPlaying() {
  gameManagerTestHelper.asyncAssertAndSignalChangePlayerStateWithData("Player1", chrome.cast.games.PlayerState.PLAYING, PLAYING_DATA, PLAYER_DATA, chrome.cast.games.GameplayState.SHOWING_INFO_SCREEN, chrome.cast.games.LobbyState.CLOSED);
  asyncTestCase.waitForSignals(1, "Waiting for player to start playing");
}
function testShowInstructions() {
  gameManagerTestHelper.asyncAssertAndSignalGameplayStateChanged(chrome.cast.games.GameplayState.SHOWING_INFO_SCREEN);
  gameManagerTestHelper.asyncAssertAndSignalGameDataChanged(INSTRUCTIONS_GAME_DATA);
  asyncTestCase.waitForSignals(2, "Waiting for game instructions");
}
function testShowGameScreen() {
  gameManagerTestHelper.asyncAssertAndSignalGameplayStateChanged(chrome.cast.games.GameplayState.RUNNING);
  gameManagerTestHelper.asyncAssertAndSignalGameDataChanged(PLAYER_ACTION_GAME_DATA);
  asyncTestCase.waitForSignals(2, "Waiting for game to begin");
}
function testProcessPlayerAndEnemyMove() {
  gameManagerTestHelper.asyncAssertAndSignalSendGameMessage("Player1", SPELL_MESSAGE, chrome.cast.games.PlayerState.PLAYING, chrome.cast.games.GameplayState.RUNNING, chrome.cast.games.LobbyState.CLOSED);
  gameManagerTestHelper.asyncAssertAndSignalGameDataChanged(PLAYER_RESOLUTION_GAME_DATA);
  gameManagerTestHelper.asyncAssertAndSignalGameDataChanged(ENEMY_RESOLUTION_GAME_DATA);
  asyncTestCase.waitForSignals(3, "Waiting to process player and enemy moves");
}
function testKillPlayer() {
  gameManagerTestHelper.asyncAssertAndSignalKillPlayer("Player1");
  asyncTestCase.waitForSignals(1, "Waiting to kill player");
}
