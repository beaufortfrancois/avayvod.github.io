...
castv2testing.e2e.Controller.prototype.stopMedia = function(successCallback, errorCallback) {
  this.media_ ? (this.logger_.info("Stopping " + this.media_.sessionId + ":" + this.media_.mediaSessionId), this.media_.stop(null, this.mediaCommandSuccessCallback_.bind(this, "media stopped", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.severe("No media"), errorCallback && errorCallback());
};
...
castv2testing.e2e.Controller.prototype.queueLoad = function(mediaUrls, currentTime, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("loading " + mediaUrls);
    for (var items = [], i = 0;i < mediaUrls.length;i++) {
      var mediaInfo = new chrome.cast.media.MediaInfo(mediaUrls[i], "video/mp4");
      items.push(new chrome.cast.media.QueueItem(mediaInfo));
    }
    var request = new chrome.cast.media.QueueLoadRequest(items);
    request.currentTime = currentTime;
    this.session_.queueLoad(request, this.onLoadMediaSuccess_.bind(this, successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.queueInsertItems = function(mediaUrls, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("inserting " + mediaUrls);
    for (var items = [], i = 0;i < mediaUrls.length;i++) {
      var mediaInfo = new chrome.cast.media.MediaInfo(mediaUrls[i], "video/mp4");
      items.push(new chrome.cast.media.QueueItem(mediaInfo));
    }
    var request = new chrome.cast.media.QueueInsertItemsRequest(items);
    this.media_.queueInsertItems(request, this.mediaCommandSuccessCallback_.bind(this, "queue insert done", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.queueUpdateItems = function(itemsToUpdate, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("updating queue");
    var request = new chrome.cast.media.QueueUpdateItemsRequest(itemsToUpdate);
    this.media_.queueUpdateItems(request, this.mediaCommandSuccessCallback_.bind(this, "queue update done", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.queueRemoveItems = function(itemIds, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("removing items " + itemIds);
    var request = new chrome.cast.media.QueueRemoveItemsRequest(itemIds);
    this.media_.queueRemoveItems(request, this.mediaCommandSuccessCallback_.bind(this, "queue remove done", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.queueReorderItems = function(itemIds, insertBefore, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("reordering items " + itemIds);
    var request = new chrome.cast.media.QueueReorderItemsRequest(itemIds);
    request.insertBefore = insertBefore;
    this.media_.queueReorderItems(request, this.mediaCommandSuccessCallback_.bind(this, "queue reorder done", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.queuePrev = function(successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("jumping to the previous item"), this.media_.queuePrev(this.mediaCommandSuccessCallback_.bind(this, "queue prev done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueNext = function(successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("jumping to the next item"), this.media_.queueNext(this.mediaCommandSuccessCallback_.bind(this, "queue next done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueSetRepeatMode = function(repeatMode, successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("setting repeat mode to " + repeatMode), this.media_.queueSetRepeatMode(repeatMode, this.mediaCommandSuccessCallback_.bind(this, "queue set repeat mode done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueRemoveItem = function(itemId, successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("removing item with " + itemId), this.media_.queueRemoveItem(itemId, this.mediaCommandSuccessCallback_.bind(this, "queue remove item done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueJumpToItem = function(itemId, successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("jumping to item with " + itemId), this.media_.queueJumpToItem(itemId, this.mediaCommandSuccessCallback_.bind(this, "queue jump to item done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueJumpToItemWithCurrentTime = function(itemId, currentTime, successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("jumping to item with " + itemId), this.media_.queueJumpToItemWithCurrentTime(itemId, currentTime, this.mediaCommandSuccessCallback_.bind(this, "queue jump to item done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueMoveItemToNewIndex = function(itemId, newIndex, successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("moving item with " + itemId + " to new index " + newIndex), this.media_.queueMoveItemToNewIndex(itemId, newIndex, this.mediaCommandSuccessCallback_.bind(this, "queue move item to new index done", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueAppendItem = function(item, successCallback, errorCallback) {
  this.session_ ? (this.logger_.info("appending item"), this.media_.queueAppendItem(item, this.mediaCommandSuccessCallback_.bind(this, "queue append item", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.queueInsertAndPlayItem = function(item, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("insert and play item");
    var request = new chrome.cast.media.QueueInsertAndPlayItemRequest(item);
    this.media_.queueInsertAndPlayItem(request, this.mediaCommandSuccessCallback_.bind(this, "queue insert and play item", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
...
castv2testing.e2e.Controller.prototype.onLoadMediaSuccess_ = function(callback, media) {
  this.logger_.info("New media session ID: " + media.mediaSessionId);
  this.media_ = media;
  callback && callback(media);
};
castv2testing.e2e.Controller.prototype.mediaCommandSuccessCallback_ = function(info, callback) {
...
var controller = new castv2testing.e2e.Controller, mediaUrl = null, initSuccessful = !1, isReceiverAvailable = !1;
function setUpPage() {
  asyncTestCase.setTests([new goog.testing.TestCase.Test("testLoadAPIScript", testLoadAPIScript), new goog.testing.TestCase.Test("testInitialization", testInitialization), new goog.testing.TestCase.Test("testRequestSession", testRequestSession), new goog.testing.TestCase.Test("testQueueLoad", testQueueLoad), new goog.testing.TestCase.Test("testQueueInsertItems", testQueueInsertItems), new goog.testing.TestCase.Test("testQueueUpdateItems", testQueueUpdateItems), new goog.testing.TestCase.Test("testQueueRemoveItems", 
  testQueueRemoveItems), new goog.testing.TestCase.Test("testQueueReorderItems", testQueueReorderItems), new goog.testing.TestCase.Test("testQueuePrev", testQueuePrev), new goog.testing.TestCase.Test("testQueueSetRepeatMode", testQueueSetRepeatMode), new goog.testing.TestCase.Test("testQueueNext", testQueueNext), new goog.testing.TestCase.Test("testQueueRemoveItem", testQueueRemoveItem), new goog.testing.TestCase.Test("testQueueJumpToItem", testQueueJumpToItem), new goog.testing.TestCase.Test("testQueueMoveItemToNewIndex", 
  testQueueMoveItemToNewIndex), new goog.testing.TestCase.Test("testQueueJumpToItemWithCurrentTime", testQueueJumpToItemWithCurrentTime), new goog.testing.TestCase.Test("testQueueAppendItem", testQueueAppendItem), new goog.testing.TestCase.Test("testQueueInsertAndPlayItem", testQueueInsertAndPlayItem), new goog.testing.TestCase.Test("testStopMedia", testStopMedia), new goog.testing.TestCase.Test("testStopSession", testStopSession)]);
}
...
function testInitialization() {
  controller.initialize("BE6E4473", null, function(availability) {
    initSuccessful && !isReceiverAvailable && availability == chrome.cast.ReceiverAvailability.AVAILABLE && (isReceiverAvailable = !0, asyncTestCase.continueTesting());
  }, function() {
    initSuccessful = !0;
  }, function(error) {
...
function testRequestSession() {
  ...
  mediaUrl = castv2testing.e2e.Utilities.GetMediaUrl();
  asyncTestCase.waitForAsync("Waiting for session request finish");
}
function testQueueLoad() {
  controller.queueLoad([mediaUrl, mediaUrl, mediaUrl], 100, function(media) {
    assertNotNullNorUndefined(media);
    assertNotNullNorUndefined(media.mediaSessionId);
    assertEquals(media, controller.session_.media[0]);
    assertEquals(3, media.items.length);
    assertEquals(chrome.cast.media.RepeatMode.OFF, media.repeatMode);
    assertEquals(100, media.currentTime);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to load queue");
  });
  asyncTestCase.waitForAsync("Waiting for loading queue");
}
function testQueueInsertItems() {
  var expectedNumItems = controller.session_.media[0].items.length + 3;
  controller.queueInsertItems([mediaUrl, mediaUrl, mediaUrl], function() {
    var media = controller.session_.media[0];
    assertEquals(expectedNumItems, media.items.length);
    assertEquals(chrome.cast.media.RepeatMode.OFF, media.repeatMode);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to insert into queue");
  });
  asyncTestCase.waitForAsync("Waiting for inserting into queue");
}
function testQueueUpdateItems() {
  var itemsToUpdate = [], firstItem = controller.session_.media[0].items[0];
  firstItem.autoplay = !1;
  itemsToUpdate.push(firstItem);
  var expectedNumItems = controller.session_.media[0].items.length;
  controller.queueUpdateItems(itemsToUpdate, function() {
    var media = controller.session_.media[0];
    assertEquals(expectedNumItems, media.items.length);
    assertFalse(media.items[0].autoplay);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to update the queue");
  });
  asyncTestCase.waitForAsync("Waiting for updating the queue");
}
function testQueueRemoveItems() {
  var expectedNumItems = controller.session_.media[0].items.length - 2, itemIdsToRemove = [], items = controller.session_.media[0].items;
  itemIdsToRemove.push(items[0].itemId);
  itemIdsToRemove.push(items[1].itemId);
  controller.queueRemoveItems(itemIdsToRemove, function() {
    assertEquals(expectedNumItems, controller.session_.media[0].items.length);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to remove items from the queue");
  });
  asyncTestCase.waitForAsync("Waiting for removing items from the queue");
}
function testQueueReorderItems() {
  var expectedNumItems = controller.session_.media[0].items.length, items = controller.session_.media[0].items, itemIdsToReorder = [];
  itemIdsToReorder.push(items[1].itemId);
  itemIdsToReorder.push(items[0].itemId);
  var itemIdToInsertBefore = items[2].itemId, expectedItemIds = [];
  expectedItemIds.push(items[1].itemId);
  expectedItemIds.push(items[0].itemId);
  for (var i$jscomp$0 = 2;i$jscomp$0 < expectedNumItems;i$jscomp$0++) {
    expectedItemIds.push(items[i$jscomp$0].itemId);
  }
  controller.queueReorderItems(itemIdsToReorder, itemIdToInsertBefore, function() {
    var media = controller.session_.media[0];
    assertEquals(expectedNumItems, media.items.length);
    for (var i = 0;i < expectedNumItems;i++) {
      assertEquals(expectedItemIds[i], media.items[i].itemId);
    }
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to reorder the queue");
  });
  asyncTestCase.waitForAsync("Waiting for reordering the queue");
}
function testQueuePrev() {
  for (var currentItemId = controller.session_.media[0].currentItemId, items = controller.session_.media[0].items, expectedLoadingItemId = null, i = 0;i < items.length;i++) {
    if (currentItemId == items[i].itemId) {
      expectedLoadingItemId = 0 == i ? items[items.length - 1].itemId : items[i - 1].itemId;
      break;
    }
  }
  assertNotNullNorUndefined(expectedLoadingItemId);
  controller.queuePrev(function() {
    var media = controller.session_.media[0];
    assertEquals(currentItemId, media.currentItemId);
    assertEquals(expectedLoadingItemId, media.loadingItemId);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to jump to the previous item");
  });
  asyncTestCase.waitForAsync("Waiting for jumping prev in the queue");
}
function testQueueNext() {
  for (var currentItemId = controller.session_.media[0].currentItemId, items = controller.session_.media[0].items, expectedLoadingItemId = null, i = 0;i < items.length;i++) {
    if (currentItemId == items[i].itemId) {
      expectedLoadingItemId = i == items.length - 1 ? items[0].itemId : items[i + 1].itemId;
      break;
    }
  }
  assertNotNullNorUndefined(expectedLoadingItemId);
  controller.queueNext(function() {
    var media = controller.session_.media[0];
    assertEquals(currentItemId, media.currentItemId);
    assertEquals(expectedLoadingItemId, media.loadingItemId);
    setTimeout(function() {
    ...
    fail("Fail to jump to the next item");
  });
  asyncTestCase.waitForAsync("Waiting for jumping next in the queue");
}
function testQueueSetRepeatMode() {
  var repeat_modes = [chrome.cast.media.RepeatMode.OFF, chrome.cast.media.RepeatMode.SINGLE, chrome.cast.media.RepeatMode.ALL_AND_SHUFFLE, chrome.cast.media.RepeatMode.ALL], change_repeat_mode = function(index) {
    var repeat_mode = repeat_modes[index];
    controller.queueSetRepeatMode(repeat_mode, function() {
      assertEquals(repeat_mode, controller.session_.media[0].repeatMode);
      setTimeout(function() {
        index == repeat_modes.length - 1 ? asyncTestCase.continueTesting() : change_repeat_mode(index + 1);
      }, 5000);
    }, function(error) {
      fail("Fail to set repeatMode " + index);
    });
  };
  change_repeat_mode(0);
  asyncTestCase.waitForAsync("Waiting to set the repeat modes.");
}
function testQueueRemoveItem() {
  for (var currentItemId = controller.session_.media[0].currentItemId, items = controller.session_.media[0].items, expectedNumItemsAfterRemoval = items.length - 1, expectedItemIdsAfterRemoval = [], i$jscomp$0 = 0;i$jscomp$0 < items.length;i$jscomp$0++) {
    items[i$jscomp$0].itemId != currentItemId && expectedItemIdsAfterRemoval.push(items[i$jscomp$0].itemId);
  }
  assertEquals(expectedNumItemsAfterRemoval, expectedItemIdsAfterRemoval.length);
  controller.queueRemoveItem(currentItemId, function() {
    var media = controller.session_.media[0];
    assertEquals(expectedNumItemsAfterRemoval, media.items.length);
    for (var i = 0;i < expectedNumItemsAfterRemoval;i++) {
      assertEquals(expectedItemIdsAfterRemoval[i], media.items[i].itemId);
    }
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to remove item");
  });
  asyncTestCase.waitForAsync("Waiting for removing item from the queue");
}
function testQueueJumpToItem() {
  var media$jscomp$0 = controller.session_.media[0], currentItemId = media$jscomp$0.currentItemId, lastItemId = media$jscomp$0.items[media$jscomp$0.items.length - 1].itemId;
  controller.queueJumpToItem(lastItemId, function() {
    var media = controller.session_.media[0];
    assertEquals(currentItemId, media.currentItemId);
    assertEquals(lastItemId, media.loadingItemId);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to jump to an item");
  });
  asyncTestCase.waitForAsync("Waiting for jumping to an item in the queue");
}
function testQueueJumpToItemWithCurrentTime() {
  if (controller.media_.queueJumpToItemWithCurrentTime) {
    var media$jscomp$0 = controller.session_.media[0], currentItemId = media$jscomp$0.currentItemId, firstItemId = media$jscomp$0.items[0].itemId;
    controller.queueJumpToItemWithCurrentTime(firstItemId, 50, function() {
      var media = controller.session_.media[0];
      assertEquals(currentItemId, media.currentItemId);
      assertEquals(firstItemId, media.loadingItemId);
      setTimeout(function() {
        asyncTestCase.continueTesting();
      }, 10000);
    }, function(error) {
...
function testQueueMoveItemToNewIndex() {
  for (var items = controller.session_.media[0].items, firstItemId = items[0].itemId, newIndex = items.length - 1, expectedItemIdsAfterMove = [], i$jscomp$0 = 1;i$jscomp$0 < items.length;i$jscomp$0++) {
    expectedItemIdsAfterMove.push(items[i$jscomp$0].itemId);
  }
  expectedItemIdsAfterMove.push(firstItemId);
  var expectedNumItemsAfterMove = items.length;
  controller.queueMoveItemToNewIndex(firstItemId, newIndex, function() {
    var media = controller.session_.media[0];
    assertEquals(expectedNumItemsAfterMove, media.items.length);
    for (var i = 0;i < media.items.length;i++) {
      assertEquals(expectedItemIdsAfterMove[i], media.items[i].itemId);
    }
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to move an item to new index");
  });
  asyncTestCase.waitForAsync("Waiting for moving an item to a new index in the queue");
}
function testQueueAppendItem() {
  var items = controller.session_.media[0].items, expectedNumItems = items.length + 1, firstItem = items[0];
  firstItem.itemId = null;
  controller.queueAppendItem(firstItem, function() {
    assertEquals(expectedNumItems, controller.session_.media[0].items.length);
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to append an item");
  });
  asyncTestCase.waitForAsync("Waiting for appending an item to the end of the queue");
}
function testQueueInsertAndPlayItem() {
  if (controller.media_.queueInsertAndPlayItem) {
    var media$jscomp$0 = controller.session_.media[0], items = media$jscomp$0.items, currentItemId = media$jscomp$0.currentItemId, expectedLoadingItemId = media$jscomp$0.items[media$jscomp$0.items.length - 1].itemId + 1, expectedNumItems = items.length + 1, firstItem = items[0];
    firstItem.itemId = null;
    controller.queueInsertAndPlayItem(firstItem, function() {
      var media = controller.session_.media[0];
      assertEquals(expectedNumItems, media.items.length);
      assertEquals(currentItemId, media.currentItemId);
      assertEquals(expectedLoadingItemId, media.loadingItemId);
...
function testStopMedia() {
  controller.stopMedia(function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertEquals(chrome.cast.media.PlayerState.IDLE, media.playerState);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to stop media");
  });
  asyncTestCase.waitForAsync("Waiting for stoping media");
}
function testStopSession() {
  controller.stopSession(function(session) {
    asyncTestCase.continueTesting();
  }, function(error) {
    isBling() ? asyncTestCase.continueTesting() : fail("Fail to stop session, error message" + JSON.stringify(error));
  });
