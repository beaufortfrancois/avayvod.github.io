...
}, assertArrayEquals = function(a, b, opt_c) {
  _validateArguments(2, arguments);
...
castv2testing.e2e.Controller.prototype.loadMedia = function(mediaUrl, autoplay, successCallback, errorCallback) {
  if (this.session_) {
    this.logger_.info("loading " + mediaUrl);
    var mediaInfo = new chrome.cast.media.MediaInfo(mediaUrl, "video/mp4");
    goog.object.containsKey(mediaInfo, "tracks") && (mediaInfo.tracks = [new chrome.cast.media.Track(0, chrome.cast.media.TrackType.TEXT), new chrome.cast.media.Track(1, chrome.cast.media.TrackType.TEXT)], mediaInfo.tracks[0].trackContentId = "track0.vtt", mediaInfo.tracks[1].trackContentId = "track1.vtt");
    var request = new chrome.cast.media.LoadRequest(mediaInfo);
    request.autoplay = autoplay;
    request.currentTime = 0;
    goog.object.containsKey(request, "activeTrackIds") && (request.activeTrackIds = [0]);
    this.session_.loadMedia(request, this.onLoadMediaSuccess_.bind(this, successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.info("Cannot load media without a live session"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.editMediaTrackInfo = function(activeTrackIds, successCallback, errorCallback) {
  if (this.media_) {
    this.logger_.info("Editing track info " + this.media_.sessionId + ":" + this.media_.mediaSessionId + " to " + activeTrackIds);
    var request = new chrome.cast.media.EditTracksInfoRequest(activeTrackIds);
    this.media_.editTracksInfo(request, this.mediaCommandSuccessCallback_.bind(this, "edited tracks info", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.severe("No media"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.getMediaStatus = function(successCallback, errorCallback) {
  this.media_ ? (this.logger_.info("Getting status " + this.media_.sessionId + ":" + this.media_.mediaSessionId), this.media_.getStatus(null, this.mediaCommandSuccessCallback_.bind(this, "got media status", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.severe("No media"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.pauseMedia = function(successCallback, errorCallback) {
  this.media_ ? (this.logger_.info("Pausing " + this.media_.sessionId + ":" + this.media_.mediaSessionId), this.media_.pause(null, this.mediaCommandSuccessCallback_.bind(this, "media paused", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.severe("No media"), errorCallback && errorCallback());
};
castv2testing.e2e.Controller.prototype.playMedia = function(successCallback, errorCallback) {
  this.media_ ? (this.logger_.info("Resuming " + this.media_.sessionId + ":" + this.media_.mediaSessionId), this.media_.play(null, this.mediaCommandSuccessCallback_.bind(this, "media playing", successCallback), this.onError_.bind(this, errorCallback))) : (this.logger_.severe("No media"), errorCallback && errorCallback());
};
...
castv2testing.e2e.Controller.prototype.seekMedia = function(mediaCurrentTime, successCallback, errorCallback) {
  if (this.media_) {
    this.logger_.info("Seeking " + this.media_.sessionId + ":" + this.media_.mediaSessionId + " to " + mediaCurrentTime);
    var request = new chrome.cast.media.SeekRequest;
    request.currentTime = mediaCurrentTime;
    this.media_.seek(request, this.mediaCommandSuccessCallback_.bind(this, "media seek done", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.severe("No media"), errorCallback && errorCallback();
  }
};
castv2testing.e2e.Controller.prototype.setMediaVolume = function(mediaLevel, muteMedia, successCallback, errorCallback) {
  if (this.media_) {
    this.logger_.info("Setting media " + this.media_.sessionId + ":" + this.media_.mediaSessionId + " volume to " + mediaLevel);
    var volume = new chrome.cast.Volume;
    volume.level = mediaLevel;
    volume.muted = muteMedia;
    var request = new chrome.cast.media.VolumeRequest;
    request.volume = volume;
    this.media_.setVolume(request, this.mediaCommandSuccessCallback_.bind(this, "media set-volume done", successCallback), this.onError_.bind(this, errorCallback));
  } else {
    this.logger_.severe("No media"), errorCallback && errorCallback();
  }
};
...
castv2testing.e2e.Controller.prototype.supportsCommand = function(command) {
  if (this.media_) {
    return this.media_.supportsCommand(command);
  }
  this.logger_.info("No media available");
  return !1;
};
...
function setUpPage() {
  asyncTestCase.setTests([new goog.testing.TestCase.Test("testLoadAPIScript", testLoadAPIScript), new goog.testing.TestCase.Test("testInitialization", testInitialization), new goog.testing.TestCase.Test("testRequestSession", testRequestSession), new goog.testing.TestCase.Test("testLoadMedia", testLoadMedia), new goog.testing.TestCase.Test("testEditTracksInfo", testEditTracksInfo), new goog.testing.TestCase.Test("testGetMediaStatus", testGetMediaStatus), new goog.testing.TestCase.Test("testPauseMedia", 
  testPauseMedia), new goog.testing.TestCase.Test("testPlayMedia", testPlayMedia), new goog.testing.TestCase.Test("testMuteVolume", testMuteVolume), new goog.testing.TestCase.Test("testSetMediaVolume", testSetMediaVolume), new goog.testing.TestCase.Test("testMuteDeviceVolume", testMuteDeviceVolume), new goog.testing.TestCase.Test("testUnMuteDeviceVolume", testUnMuteDeviceVolume), new goog.testing.TestCase.Test("testSetDeviceVolume", testSetDeviceVolume), new goog.testing.TestCase.Test("testSeekMedia", 
  testSeekMedia), new goog.testing.TestCase.Test("testSupportsCommand", testSupportsCommand), new goog.testing.TestCase.Test("testStopMedia", testStopMedia), new goog.testing.TestCase.Test("testStopSession", testStopSession)]);
}
...
function testLoadMedia() {
  controller.loadMedia(mediaUrl, !0, function(media) {
    ...
    if (controller.media_.editTracksInfo) {
      assertArrayEquals([0], media.activeTrackIds);
      var trackIds = media.media.tracks.map(function(track) {
        return track.trackId;
      });
      assertArrayEquals([0, 1], trackIds);
    }
    setTimeout(function() {
      asyncTestCase.continueTesting();
    }, 10000);
  }, function(error) {
    fail("Fail to load media");
  });
  asyncTestCase.waitForAsync("Waiting for loading media");
}
function testEditTracksInfo() {
  !isBling() && controller.media_.editTracksInfo && (assertArrayEquals([0], controller.media_.activeTrackIds), controller.editMediaTrackInfo([1], function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertArrayEquals([1], media.activeTrackIds);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to edit track info");
  }), asyncTestCase.waitForAsync("Waiting for edit track info"));
}
function testGetMediaStatus() {
  controller.media_.getStatus && (controller.getMediaStatus(function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertTrue(media.playerState == chrome.cast.media.PlayerState.PLAYING || media.playerState == chrome.cast.media.PlayerState.BUFFERING);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to get media status");
  }), asyncTestCase.waitForAsync("Waiting for get media status"));
}
function testPauseMedia() {
  controller.pauseMedia(function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertEquals(chrome.cast.media.PlayerState.PAUSED, media.playerState);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to pause media");
  });
  asyncTestCase.waitForAsync("Waiting for pausing media");
}
function testPlayMedia() {
  controller.playMedia(function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertTrue(media.playerState == chrome.cast.media.PlayerState.PLAYING || media.playerState == chrome.cast.media.PlayerState.BUFFERING);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to play media");
  });
  asyncTestCase.waitForAsync("Waiting for playing media");
}
function testMuteVolume() {
  controller.setMediaVolume(0.1, !0, function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertTrue(media.volume.muted);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to mute media volume");
  });
  asyncTestCase.waitForAsync("Waiting for muting media volume");
}
function testSetMediaVolume() {
  controller.setMediaVolume(0.2, !1, function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertEquals(0.2, media.volume.level);
    assertFalse(media.volume.muted);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to set media volume");
  });
  asyncTestCase.waitForAsync("Waiting for setting media volume");
}
function testMuteDeviceVolume() {
  controller.session_.receiver.isActiveInput && (controller.setDeviceVolume(void 0, !0, function() {
    var session = controller.session_;
    assertNotNullNorUndefined(session);
    assertNotNullNorUndefined(session.receiver.volume);
    console.log("device volume: " + JSON.stringify(session.receiver.volume));
    assertTrue(session.receiver.volume.muted);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to mute device volume");
  }), asyncTestCase.waitForAsync("Waiting for muting device volume"));
}
function testUnMuteDeviceVolume() {
  controller.session_.receiver.isActiveInput && (controller.setDeviceVolume(void 0, !1, function() {
    var session = controller.session_;
    assertNotNullNorUndefined(session);
    assertNotNullNorUndefined(session.receiver.volume);
    assertFalse(session.receiver.volume.muted);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to unmute device volume");
  }), asyncTestCase.waitForAsync("Waiting for unmuting device volume"));
}
function testSetDeviceVolume() {
  controller.session_.receiver.isActiveInput && (controller.setDeviceVolume(0.4, !1, function() {
    var session = controller.session_;
    assertNotNullNorUndefined(session);
    assertNotNullNorUndefined(session.receiver.volume);
    console.log("device volume: " + JSON.stringify(session.receiver.volume));
    assertTrue(0.01 > Math.abs(session.receiver.volume.level - 0.4));
    assertFalse(session.receiver.volume.muted);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to set device volume");
  }), asyncTestCase.waitForAsync("Waiting for setting device volume"));
}
function testSeekMedia() {
  controller.seekMedia(60, function() {
    var media = controller.media_;
    assertNotNullNorUndefined(media);
    assertEquals(60, media.currentTime);
    asyncTestCase.continueTesting();
  }, function(error) {
    fail("Fail to seek media");
  });
  asyncTestCase.waitForAsync("Waiting for seeking media");
}
function testSupportsCommand() {
  assertTrue(controller.supportsCommand(chrome.cast.media.MediaCommand.PAUSE));
  assertTrue(controller.supportsCommand(chrome.cast.media.MediaCommand.SEEK));
  assertTrue(controller.supportsCommand(chrome.cast.media.MediaCommand.STREAM_VOLUME));
  assertTrue(controller.supportsCommand(chrome.cast.media.MediaCommand.STREAM_MUTE));
}
