...
castv2testing.e2e.GameManagerTestHelper.prototype.asyncAssertAndSignalPlayerStateTransitions = function(playerId, transitionStates) {
  this.changeAndAssertPlayerStates_(playerId, transitionStates, 0);
};
castv2testing.e2e.GameManagerTestHelper.prototype.changeAndAssertPlayerStates_ = function(playerId, transitionStates, index) {
  var successCallback = this.assertCurrentPlayerState_.bind(this, playerId, transitionStates, index), errorCallback = function(error) {
    fail("Transition failure to " + this.gameManagerController_.getNameForValue(chrome.cast.games.PlayerState, transitionStates[index]) + " : " + JSON.stringify(error));
  }.bind(this);
  (0,this.stateChangeFunctions_[transitionStates[index]])(playerId, null, successCallback, errorCallback);
};
castv2testing.e2e.GameManagerTestHelper.prototype.assertCurrentPlayerState_ = function(playerId, transitionStates, index) {
  var playerState = transitionStates[index], currentState = this.gameManagerController_.getState(), currentPlayer = this.assertStateAndGetPlayer(playerId), playersInCurrentState = currentState.getPlayersInState(playerState);
  assertEquals(currentPlayer.getPlayerState(), playerState);
  assertNotNullNorUndefined(playersInCurrentState);
  index == transitionStates.length - 1 ? this.asyncTestCase_.signal() : this.changeAndAssertPlayerStates_(playerId, transitionStates, index + 1);
};
...
var PLAYER_MESSAGE1 = {fire:!0, move:0.1}, PLAYER_MESSAGE2 = {fire:!1, move:0.5}, controller = null, gameManagerController = null, gameManagerTestHelper = null;
function setUpPage() {
  ...
  asyncTestCase.setTests([new goog.testing.TestCase.Test("testSetupSession", testSetupSession), new goog.testing.TestCase.Test("testSetupGameManagerClient", testSetupGameManagerClient), new goog.testing.TestCase.Test("testWaitForRunningGame", testWaitForRunningGame), new goog.testing.TestCase.Test("testCreatePlayers", testCreatePlayers), new goog.testing.TestCase.Test("testGameMessages", testGameMessages), new goog.testing.TestCase.Test("testValidPlayerTransitions", testValidPlayerTransitions), new goog.testing.TestCase.Test("testKillPlayers", 
  testKillPlayers), new goog.testing.TestCase.Test("testTeardown", testTeardown)]);
}
function testSetupSession() {
  gameManagerTestHelper.asyncAssertAndSignalLoadAPIScript();
  gameManagerTestHelper.asyncAssertAndSignalInitializeController("54E247D6");
...
function testCreatePlayers() {
  ...
  gameManagerTestHelper.asyncAssertAndSignalCreatePlayer("Player2", chrome.cast.games.PlayerState.PLAYING, 2);
  asyncTestCase.waitForSignals(2, "Waiting for players to be created");
}
function testGameMessages() {
  gameManagerTestHelper.asyncAssertAndSignalSendGameMessage("Player1", PLAYER_MESSAGE1, chrome.cast.games.PlayerState.PLAYING, chrome.cast.games.GameplayState.RUNNING, chrome.cast.games.LobbyState.OPEN);
  gameManagerTestHelper.asyncAssertAndSignalSendGameMessage("Player2", PLAYER_MESSAGE2, chrome.cast.games.PlayerState.PLAYING, chrome.cast.games.GameplayState.RUNNING, chrome.cast.games.LobbyState.OPEN);
  asyncTestCase.waitForSignals(2, "Waiting for game messages to be sent");
}
function testValidPlayerTransitions() {
  var playing = chrome.cast.games.PlayerState.PLAYING, idle = chrome.cast.games.PlayerState.IDLE, transitionStates = [chrome.cast.games.PlayerState.READY, playing, idle, playing, idle, playing, idle, playing, idle];
  gameManagerTestHelper.asyncAssertAndSignalPlayerStateTransitions("Player1", transitionStates);
  gameManagerTestHelper.asyncAssertAndSignalPlayerStateTransitions("Player2", transitionStates);
  asyncTestCase.waitForSignals(2, "Waiting for transition tests to complete");
}
function testKillPlayers() {
  gameManagerTestHelper.asyncAssertAndSignalKillPlayer("Player1");
  gameManagerTestHelper.asyncAssertAndSignalKillPlayer("Player2");
  asyncTestCase.waitForSignals(2, "Waiting to kill players");
}
