...
chrome.cast.games.internal.RequestIdGenerator = function() {
  this.nextRequestId_ = Math.floor(Math.random() * 1e5) * 1000;
};
goog.addSingletonGetter(chrome.cast.games.internal.RequestIdGenerator);
chrome.cast.games.internal.RequestIdGenerator.prototype.getNextRequestId = function() {
  var nextId = this.nextRequestId_++;
  if (nextId == 0) {
    nextId = this.nextRequestId_++;
  }
  return nextId;
};
...
chrome.cast.games.internal.GameManagerRequest = function() {
  ...
  this.requestId = chrome.cast.games.internal.RequestIdGenerator.getInstance().getNextRequestId();
  this.playerId = "";
...
chrome.cast.games.GameManagerError = function(errorCode, errorDescription, result, castError) {
...
chrome.cast.games.GameManagerResult = function(playerId, requestId, extraMessageData) {
...
{
  a: {
    var doc$jscomp$inline_3 = goog.global.document;
    if (!doc$jscomp$inline_3 || !goog.userAgent.IE) {
      JSCompiler_inline_result$jscomp$2 = undefined;
      break a;
    }
    JSCompiler_inline_result$jscomp$2 = goog.userAgent.getDocumentMode_() || (doc$jscomp$inline_3.compatMode == "CSS1Compat" ? parseInt(goog.userAgent.VERSION, 10) : 5);
  }
}
...
goog.debug.LogRecord.prototype.reset = function(level, msg, loggerName, opt_time, opt_sequenceNumber) {
  if (goog.debug.LogRecord.ENABLE_SEQUENCE_NUMBERS) {
    typeof opt_sequenceNumber == "number" ? opt_sequenceNumber : goog.debug.LogRecord.nextSequenceNumber_++;
  }
  opt_time || goog.now();
  this.level_ = level;
  this.msg_ = msg;
...
chrome.cast.games.internal.GameManagerSession = function(session) {
  ...
  this.logger_ = goog.log.getLogger("chrome.cast.games.internal.GameManagerSession");
  this.initializationSuccessCallback_ = null;
  this.successCallbackMap_ = new goog.structs.Map;
  this.errorCallbackMap_ = new goog.structs.Map;
  this.requestTimeoutIdMap_ = new goog.structs.Map;
  this.requestGameManagerRequestTypeMap_ = new goog.structs.Map;
  this.playerTokenMap_ = new goog.structs.Map;
  this.lastUsedPlayerId_ = this.postUpdateStateMessageCallback_ = this.updateStateMessageCallback_ = null;
  this.session_.addMessageListener(this.getNamespace(), goog.bind(this.onMessage_, this));
};
chrome.cast.games.internal.GameManagerSession.TIMEOUT_LIMIT = 60000;
chrome.cast.games.internal.GameManagerSession.prototype.dispose = function() {
  this.session_.removeMessageListener(chrome.cast.games.NAMESPACE, goog.bind(this.onMessage_, this));
  for (var requestIds = this.requestTimeoutIdMap_.getKeys(), i = 0;i < requestIds.length;i++) {
    this.removeResponse_(requestIds[i]);
  }
  this.postUpdateStateMessageCallback_ = this.updateStateMessageCallback_ = null;
};
chrome.cast.games.internal.GameManagerSession.prototype.getNamespace = function() {
  return chrome.cast.games.NAMESPACE;
};
chrome.cast.games.internal.GameManagerSession.prototype.sendGameManagerInitializeRequest = function(initializationSuccessCallback, errorCallback) {
  var request = new chrome.cast.games.internal.GameManagerRequest;
  request.type = chrome.cast.games.internal.GameManagerRequestType.GET_GAME_MANAGER_CONFIG;
  if (!request.requestId) {
    throw Error("Missing requestId field from request.");
  }
  if (!initializationSuccessCallback) {
    throw Error("No success callback provided.");
  }
  this.initializationSuccessCallback_ = initializationSuccessCallback;
  var castErrorCallback = goog.nullFunction;
  if (errorCallback) {
    this.errorCallbackMap_.set(request.requestId, errorCallback);
    castErrorCallback = function(castError) {
      errorCallback(new chrome.cast.games.GameManagerError(chrome.cast.games.GameManagerErrorCode.CAST_ERROR, castError.description, null, castError));
      this.removeResponse_(request.requestId);
    }.bind(this);
  }
  var timeoutId = goog.global.setTimeout(goog.bind(this.onTimeout_, this, request.requestId), chrome.cast.games.internal.GameManagerSession.TIMEOUT_LIMIT);
  this.requestTimeoutIdMap_.set(request.requestId, timeoutId);
  this.requestGameManagerRequestTypeMap_.set(request.requestId, request.type);
  this.session_.sendMessage(this.getNamespace(), request, goog.nullFunction, castErrorCallback);
};
chrome.cast.games.internal.GameManagerSession.prototype.sendGameManagerRequest = function(playerId, requestType, extraMessageData, successCallback, errorCallback) {
  if (requestType == chrome.cast.games.internal.GameManagerRequestType.UNKNOWN) {
    throw Error("Cannot send an UNKNOWN game manager request.");
  }
  if (!playerId && requestType != chrome.cast.games.internal.GameManagerRequestType.PLAYER_AVAILABLE && requestType != chrome.cast.games.internal.GameManagerRequestType.GET_GAME_MANAGER_CONFIG) {
    throw Error("Missing player ID for game manager request type:" + requestType);
  }
  var request = new chrome.cast.games.internal.GameManagerRequest;
  request.type = requestType;
  request.playerId = playerId ? playerId : "";
  request.playerToken = this.playerTokenMap_.get(request.playerId, null);
  request.extraMessageData = extraMessageData;
  if (!request.requestId) {
    throw Error("Missing requestId field from request");
  }
  if (successCallback) {
    this.successCallbackMap_.set(request.requestId, successCallback);
  }
  var castErrorCallback = goog.nullFunction;
  if (errorCallback) {
    this.errorCallbackMap_.set(request.requestId, errorCallback);
  ...
  if (request.type != chrome.cast.games.internal.GameManagerRequestType.GAME_MESSAGE) {
    var timeoutId = goog.global.setTimeout(goog.bind(this.onTimeout_, this, request.requestId), chrome.cast.games.internal.GameManagerSession.TIMEOUT_LIMIT);
    this.requestTimeoutIdMap_.set(request.requestId, timeoutId);
  }
  this.requestGameManagerRequestTypeMap_.set(request.requestId, request.type);
...
chrome.cast.games.internal.GameManagerSession.prototype.setUpdateCallbacks = function(updateStateMessageCallback, postUpdateStateMessageCallback) {
  this.updateStateMessageCallback_ = updateStateMessageCallback;
  this.postUpdateStateMessageCallback_ = postUpdateStateMessageCallback;
};
chrome.cast.games.internal.GameManagerSession.prototype.isPlayerControllable = function(playerId) {
  return this.playerTokenMap_.containsKey(playerId);
};
chrome.cast.games.internal.GameManagerSession.prototype.getLastUsedPlayerId = function() {
  return this.lastUsedPlayerId_;
};
...
chrome.cast.games.internal.GameManagerSession.prototype.onMessage_ = function(namespace, message) {
  if (namespace != this.getNamespace()) {
    throw Error("Incoming message has unexpected namespace: " + namespace);
  }
  var parsed = goog.json.parse(message), type = parsed.type;
  if (type != chrome.cast.games.internal.GameManagerMessageType.GAME_MANAGER_PROCESSED_RESULT && type != chrome.cast.games.internal.GameManagerMessageType.GAME_MESSAGE) {
    goog.log.error(this.logger_, "Ignoring message, type is not GAME_MANAGER_PROCESSED_RESULT or " + " GAME_MESSAGE Type: " + type);
    return;
  }
  if (parsed.gameManagerConfig && parsed.gameManagerConfig.version != chrome.cast.games.SDK_VERSION_NUMBER) {
    parsed.statusCode = chrome.cast.games.internal.StatusCode.INCORRECT_VERSION;
    parsed.errorDescription = "Incorrect version of the GameManager SDK. " + "Sender: " + chrome.cast.games.SDK_VERSION_NUMBER + "Receiver: " + parsed.gameManagerConfig.version;
  }
  if (parsed.statusCode != chrome.cast.games.internal.StatusCode.SUCCESS) {
    this.onError_(parsed);
    return;
  }
  var playerId = parsed.playerId, playerToken = parsed.playerToken;
  if (playerId && playerToken) {
    this.playerTokenMap_.set(playerId, playerToken);
  }
  if (this.updateStateMessageCallback_) {
    this.updateStateMessageCallback_(parsed);
  }
  if (parsed.requestId) {
    this.lastUsedPlayerId_ = playerId || this.lastUsedPlayerId_;
    this.onResult_(parsed);
  }
  if (this.postUpdateStateMessageCallback_) {
    this.postUpdateStateMessageCallback_();
  }
};
chrome.cast.games.internal.GameManagerSession.prototype.onError_ = function(message) {
  var requestId = message.requestId;
  if (!this.errorCallbackMap_.containsKey(requestId)) {
    goog.log.info(this.logger_, "Ignoring error message, no callback for request ID: " + requestId);
    return;
  }
  var errorCallback = this.errorCallbackMap_.get(requestId), result = new chrome.cast.games.GameManagerResult(message.playerId, message.requestId, message.extraMessageData), errorCode = chrome.cast.games.internal.StatusCode.getGameManagerErrorCode(message.statusCode);
  errorCallback(new chrome.cast.games.GameManagerError(errorCode, message.errorDescription, result, null));
  this.removeResponse_(requestId);
};
chrome.cast.games.internal.GameManagerSession.prototype.onResult_ = function(message) {
  var requestId = message.requestId, requestType = this.requestGameManagerRequestTypeMap_.get(requestId), successCallback = this.successCallbackMap_.get(message.requestId);
  this.removeResponse_(requestId);
  if (requestType == chrome.cast.games.internal.GameManagerRequestType.GET_GAME_MANAGER_CONFIG) {
    if (!this.initializationSuccessCallback_) {
      throw Error("Got a response for a GET_GAME_MANAGER_CONFIG request " + "but no initialization callback was set.");
    }
    this.initializationSuccessCallback_();
    this.initializationSuccessCallback_ = null;
    return;
  }
  if (!successCallback) {
    goog.log.info(this.logger_, "Ignoring message, no callback for request ID: " + requestId);
    return;
  }
  successCallback(new chrome.cast.games.GameManagerResult(message.playerId, message.requestId, message.extraMessageData));
};
chrome.cast.games.internal.GameManagerSession.prototype.onTimeout_ = function(requestId) {
  var requestType = this.requestGameManagerRequestTypeMap_.get(requestId), errorDescription = "Did not receive a response to player request " + "within a time period. request ID=" + requestId + " request type =" + requestType;
  if (!this.errorCallbackMap_.containsKey(requestId)) {
    throw Error(errorDescription);
  } else {
    var callback = this.errorCallbackMap_.get(requestId), castError = new chrome.cast.Error(chrome.cast.ErrorCode.TIMEOUT, errorDescription, null);
    callback(new chrome.cast.games.GameManagerError(chrome.cast.games.GameManagerErrorCode.CAST_ERROR, errorDescription, null, castError));
    this.removeResponse_(requestId);
  }
};
chrome.cast.games.internal.GameManagerSession.prototype.removeResponse_ = function(requestId) {
  this.successCallbackMap_.remove(requestId);
  this.errorCallbackMap_.remove(requestId);
  var timeoutId = this.requestTimeoutIdMap_.get(requestId);
  goog.global.clearTimeout(timeoutId);
  this.requestTimeoutIdMap_.remove(requestId);
  this.requestGameManagerRequestTypeMap_.remove(requestId);
};
goog.disposable = {};
goog.disposable.IDisposable = function() {
};
...
goog.testing.stacktrace.parseStackFrame_ = function(frameStr) {
  ...
  if (m = frameStr.match(goog.testing.stacktrace.FIREFOX_STACK_FRAME_REGEXP_)) {
    return new goog.testing.stacktrace.Frame("", m[1] || "", "", m[3] || "");
  }
  if (m = frameStr.match(goog.testing.stacktrace.OPERA_STACK_FRAME_REGEXP_)) {
    return new goog.testing.stacktrace.Frame(m[2] || "", m[1] || m[3] || "", "", m[5] || "");
  }
  if (m = frameStr.match(goog.testing.stacktrace.IE_STACK_FRAME_REGEXP_)) {
...
var _trueTypeOf = function(something) {
  ...
  if (args.length == expectedNumberOfNonCommentArgs + 1) {
    return args[0];
  }
  return null;
}, nonCommentArg = function(desiredNonCommentArgIndex, expectedNumberOfNonCommentArgs, args) {
  return args.length == expectedNumberOfNonCommentArgs + 1 ? args[desiredNonCommentArgIndex] : args[desiredNonCommentArgIndex - 1];
...
var assert = function(a, opt_b) {
  ...
  _validateArguments(1, arguments);
  _assert(commentArg(1, arguments), nonCommentArg(1, 1, arguments) !== null, "Expected not to be " + _displayStringForValue(null));
}, assertNotUndefined = function(a, opt_b) {
  _validateArguments(1, arguments);
  _assert(commentArg(1, arguments), nonCommentArg(1, 1, arguments) !== void 0, "Expected not to be " + _displayStringForValue(void 0));
};
...
goog.testing.asserts.findDifferences = function(expected, actual, opt_equalityPredicate) {
    ...
    seen2.push(var2);
    {
      a: {
        ...
        if (typeOfVar1 == typeOfVar2) {
          var isArray = typeOfVar1 == "Array", errorMessage = equalityPredicate(typeOfVar1, var1, var2);
  ...
  var failures = [], seen1 = [], seen2 = [], equalityPredicate = opt_equalityPredicate || function(type, var1, var2) {
    ...
    return typedPredicate(var1, var2) ? goog.testing.asserts.EQUALITY_PREDICATE_VARS_ARE_EQUAL : goog.testing.asserts.getDefaultErrorMsg_(var1, var2);
  };
...
goog.exportSymbol("assertUndefined", function(a, opt_b) {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(commentArg(1, arguments), aVar === void 0, goog.testing.asserts.getDefaultErrorMsg_(void 0, aVar));
});
goog.exportSymbol("assertNotUndefined", assertNotUndefined);
goog.exportSymbol("assertNotNullNorUndefined", function(a, opt_b) {
  _validateArguments(1, arguments);
...
goog.exportSymbol("assertNonEmptyString", function(a, opt_b) {
  _validateArguments(1, arguments);
  var aVar = nonCommentArg(1, 1, arguments);
  _assert(commentArg(1, arguments), aVar !== void 0 && aVar !== null && typeof aVar == "string" && aVar !== "", "Expected non-empty string but was " + _displayStringForValue(aVar));
});
...
goog.exportSymbol("assertObjectRoughlyEquals", function(a, b, c, opt_d) {
  ...
  var tolerance = nonCommentArg(3, 3, arguments), failureMessage = commentArg(3, arguments) ? commentArg(3, arguments) : "", differences = goog.testing.asserts.findDifferences(nonCommentArg(1, 3, arguments), nonCommentArg(2, 3, arguments), function(type, var1, var2) {
    ...
    return typedPredicate(var1, var2, tolerance) ? goog.testing.asserts.EQUALITY_PREDICATE_VARS_ARE_EQUAL : goog.testing.asserts.getDefaultErrorMsg_(var1, var2) + " which was more than " + tolerance + " away";
  });
  _assert(failureMessage, !differences, differences);
});
...
goog.exportSymbol("assertEvaluatesToTrue", function(a, opt_b) {
  _validateArguments(1, arguments);
  if (!nonCommentArg(1, 1, arguments)) {
...
chrome.cast.games.GameManagerStateChangedEvent = function(currentState, previousState) {
...
chrome.cast.games.GameManagerGameMessageReceivedEvent = function(playerId, gameMessage) {
...
chrome.cast.games.GameManagerState = function(applicationName, maxPlayers, lobbyState, gameplayState, gameData, gameStatusText, players) {
  this.applicationName_ = applicationName;
  this.maxPlayers_ = maxPlayers;
  this.lobbyState_ = lobbyState;
  this.gameplayState_ = gameplayState;
  this.gameData_ = gameData;
  this.gameStatusText_ = gameStatusText;
  this.players_ = new goog.structs.Map;
  for (var i = 0;i < players.length;i++) {
    var player = players[i];
    this.players_.set(player.playerId_, player);
  }
};
...
chrome.cast.games.GameManagerState.prototype.getPlayer = function(playerId) {
  return this.players_.get(playerId, null);
};
...
chrome.cast.games.GameManagerState.prototype.getControllablePlayers = function() {
  for (var controllablePlayers = [], players = this.players_.getValues(), i = 0;i < players.length;i++) {
    var player = players[i];
    if (player.isControllable_) {
...
chrome.cast.games.GameManagerState.prototype.getConnectedPlayers = function() {
  for (var connectedPlayers = [], players = this.players_.getValues(), i = 0;i < players.length;i++) {
    var player = players[i];
...
chrome.cast.games.GameManagerState.prototype.getConnectedControllablePlayers = function() {
  for (var connectedControllablePlayers = [], players = this.players_.getValues(), i = 0;i < players.length;i++) {
    var player = players[i];
    if (player.isConnected() && player.isControllable_) {
      connectedControllablePlayers.push(player);
    }
  }
...
chrome.cast.games.GameManagerState.prototype.getPlayersInState = function(playerState) {
  for (var playersInState = [], players = this.players_.getValues(), i = 0;i < players.length;i++) {
    var player = players[i];
    if (player.playerState_ == playerState) {
      playersInState.push(player);
    }
  }
...
chrome.cast.games.GameManagerState.prototype.hasLobbyStateChanged = function(otherState) {
  return this.lobbyState_ != otherState.lobbyState_;
};
...
chrome.cast.games.GameManagerState.prototype.hasGameplayStateChanged = function(otherState) {
  return this.gameplayState_ != otherState.gameplayState_;
};
...
chrome.cast.games.GameManagerState.prototype.hasGameDataChanged = function(otherState) {
  return goog.testing.asserts.findDifferences(this.gameData_, otherState.gameData_) != null;
};
...
chrome.cast.games.GameManagerState.prototype.hasGameStatusTextChanged = function(otherState) {
  return this.gameStatusText_ != otherState.gameStatusText_;
};
...
chrome.cast.games.GameManagerState.prototype.hasPlayerChanged = function(playerId, otherState) {
  var player1 = this.getPlayer(playerId), player2 = otherState.getPlayer(playerId);
...
chrome.cast.games.GameManagerState.prototype.hasPlayerStateChanged = function(playerId, otherState) {
  var player1 = this.getPlayer(playerId), player2 = otherState.getPlayer(playerId);
  if (player1 == null && player2 == null) {
    return false;
  } else {
    if (player1 != null && player2 != null) {
      return player1.playerState_ != player2.playerState_;
    }
  }
  return true;
};
...
chrome.cast.games.GameManagerState.prototype.hasPlayerDataChanged = function(playerId, otherState) {
  var player1 = this.getPlayer(playerId), player2 = otherState.getPlayer(playerId);
  if (player1 == null && player2 == null) {
    return false;
  } else {
    if (player1 != null && player2 != null) {
      return goog.testing.asserts.findDifferences(player1.playerData_, player2.playerData_) != null;
    }
  }
  return true;
};
...
chrome.cast.games.GameManagerState.prototype.getListOfChangedPlayers = function(otherState) {
  for (var changedPlayers = [], myPlayers = this.getPlayers(), i = 0;i < myPlayers.length;i++) {
    var myPlayer = myPlayers[i], theirPlayer = otherState.getPlayer(myPlayer.playerId_);
    if (theirPlayer == null || !myPlayer.equals(theirPlayer)) {
      changedPlayers.push(myPlayer.playerId_);
    }
  }
  for (var theirPlayers = otherState.getPlayers(), i = 0;i < theirPlayers.length;i++) {
    theirPlayer = theirPlayers[i];
    myPlayer = this.getPlayer(theirPlayer.playerId_);
    if (myPlayer == null && !goog.array.contains(changedPlayers, theirPlayer.playerId_)) {
      changedPlayers.push(theirPlayer.playerId_);
    }
  }
...
chrome.cast.games.GameManagerState.prototype.equals = function(otherState) {
  return this.applicationName_ == otherState.applicationName_ && this.maxPlayers_ == otherState.maxPlayers_ && this.lobbyState_ == otherState.lobbyState_ && this.gameplayState_ == otherState.gameplayState_ && this.gameStatusText_ == otherState.gameStatusText_ && this.getListOfChangedPlayers(otherState).length == 0 && !goog.testing.asserts.findDifferences(this.gameData_, otherState.gameData_);
};
...
chrome.cast.games.PlayerInfo = function(playerId, playerState, playerData, isControllable) {
  this.playerId_ = playerId;
  this.playerState_ = playerState;
  this.playerData_ = playerData;
  this.isControllable_ = isControllable;
};
...
chrome.cast.games.PlayerInfo.prototype.isConnected = function() {
  return this.playerState_ == chrome.cast.games.PlayerState.IDLE || this.playerState_ == chrome.cast.games.PlayerState.AVAILABLE || this.playerState_ == chrome.cast.games.PlayerState.PLAYING || this.playerState_ == chrome.cast.games.PlayerState.READY;
};
...
chrome.cast.games.PlayerInfo.prototype.equals = function(other) {
  return this.playerId_ == other.playerId_ && this.playerState_ == other.playerState_ && this.isControllable_ == other.isControllable_ && !goog.testing.asserts.findDifferences(this.playerData_, other.playerData_);
};
goog.exportProperty(chrome.cast.games.PlayerInfo.prototype, "equals", chrome.cast.games.PlayerInfo.prototype.equals);
chrome.cast.games.GameManagerClient = function(session) {
  this.gameManagerSession_ = new chrome.cast.games.internal.GameManagerSession(session);
  this.eventTarget_ = new goog.events.EventTarget;
  this.previousState_ = this.currentState_ = this.gameManagerConfig_ = null;
  this.isDisposed_ = this.initializationStarted_ = false;
  this.boundOnGameManagerMessageCallback_ = goog.bind(this.onGameManagerMessage_, this);
  this.boundTriggerEvents_ = goog.bind(this.triggerEvents_, this);
  this.cachedGameMessage_ = this.cachedGameMessagePlayerRecipient_ = null;
  this.gameManagerSession_.setUpdateCallbacks(this.boundOnGameManagerMessageCallback_, this.boundTriggerEvents_);
};
goog.exportProperty(chrome.cast.games, "GameManagerClient", chrome.cast.games.GameManagerClient);
chrome.cast.games.GameManagerClient.getInstanceFor = function(session, successCallback, errorCallback) {
  (new chrome.cast.games.GameManagerClient(session)).initialize_(successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.initialize_ = function(successCallback, errorCallback) {
  if (this.initializationStarted_) {
  ...
  this.initializationStarted_ = true;
  var that = this;
  this.gameManagerSession_.sendGameManagerInitializeRequest(function() {
    successCallback(new chrome.cast.games.GameManagerInstanceResult(that));
  }, errorCallback);
};
chrome.cast.games.GameManagerClient.prototype.dispose = function() {
  ...
  this.gameManagerSession_.dispose();
  this.eventTarget_.dispose();
  this.gameManagerSession_ = null;
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerAvailableRequest = function(extraMessageData, successCallback, errorCallback) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.AVAILABLE, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerAvailableRequestWithPlayerId = function(playerId, extraMessageData, successCallback, errorCallback) {
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.AVAILABLE, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerReadyRequest = function(extraMessageData, successCallback, errorCallback) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  if (!playerId) {
  ...
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.READY, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerReadyRequestWithPlayerId = function(playerId, extraMessageData, successCallback, errorCallback) {
  if (!playerId) {
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerIdleRequest = function(extraMessageData, successCallback, errorCallback) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  if (!playerId) {
    throw Error("Cannot send player request - no last used player ID found." + "To register a new player, use setPlayerAvailableRequest()");
  }
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.IDLE, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerIdleRequestWithPlayerId = function(playerId, extraMessageData, successCallback, errorCallback) {
  if (!playerId) {
    throw Error("Cannot send player request - no player ID provided." + "To register a new player, use setPlayerAvailableRequest()");
  }
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.IDLE, extraMessageData, successCallback, errorCallback);
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerPlayingRequest = function(extraMessageData, successCallback, errorCallback) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  if (!playerId) {
    throw Error("Cannot send player request - no last used player ID found." + "To register a new player, use setPlayerAvailableRequest()");
  }
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.PLAYING, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerPlayingRequestWithPlayerId = function(playerId, extraMessageData, successCallback, errorCallback) {
  if (!playerId) {
    throw Error("Cannot send player request - no player ID provided." + "To register a new player, use setPlayerAvailableRequest()");
  }
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.PLAYING, extraMessageData, successCallback, errorCallback);
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerQuitRequest = function(extraMessageData, successCallback, errorCallback) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  if (!playerId) {
    throw Error("Cannot send player request - no last used player ID found." + "To register a new player, use setPlayerAvailableRequest()");
  }
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.QUIT, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerQuitRequestWithPlayerId = function(playerId, extraMessageData, successCallback, errorCallback) {
  if (!playerId) {
    throw Error("Cannot send player request - no player ID provided." + "To register a new player, use setPlayerAvailableRequest()");
  }
  this.sendPlayerRequest_(playerId, chrome.cast.games.PlayerState.QUIT, extraMessageData, successCallback, errorCallback);
...
chrome.cast.games.GameManagerClient.prototype.sendPlayerRequest_ = function(playerId, playerState, extraMessageData, successCallback, errorCallback) {
  this.checkState_();
  var requestType = chrome.cast.games.internal.GameManagerRequestType.getGameManagerRequestType(playerState);
  this.gameManagerSession_.sendGameManagerRequest(playerId, requestType, extraMessageData, successCallback, errorCallback);
};
chrome.cast.games.GameManagerClient.prototype.sendGameRequest = function(extraMessageData, successCallback, errorCallback) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  if (!playerId) {
    throw Error("Cannot send game request - no last used player ID found.");
  }
  this.sendGameRequestWithPlayerId(playerId, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendGameRequestWithPlayerId = function(playerId, extraMessageData, successCallback, errorCallback) {
  this.checkState_();
  this.gameManagerSession_.sendGameManagerRequest(playerId, chrome.cast.games.internal.GameManagerRequestType.GAME_REQUEST, extraMessageData, successCallback, errorCallback);
};
...
chrome.cast.games.GameManagerClient.prototype.sendGameMessage = function(extraMessageData) {
  var playerId = this.gameManagerSession_.getLastUsedPlayerId();
  if (!playerId) {
    throw Error("Cannot send game message - no last used player ID found.");
  }
  this.sendGameMessageWithPlayerId(playerId, extraMessageData);
};
...
chrome.cast.games.GameManagerClient.prototype.sendGameMessageWithPlayerId = function(playerId, extraMessageData) {
  this.checkState_();
  this.gameManagerSession_.sendGameManagerRequest(playerId, chrome.cast.games.internal.GameManagerRequestType.GAME_MESSAGE, extraMessageData, null, null);
};
...
chrome.cast.games.GameManagerClient.prototype.addEventListener = function(type, listener) {
  this.eventTarget_.listen(type, listener);
};
...
chrome.cast.games.GameManagerClient.prototype.removeEventListener = function(type, listener) {
  this.eventTarget_.unlisten(type, listener);
};
...
chrome.cast.games.GameManagerClient.prototype.getLastUsedPlayerId = function() {
  this.checkState_();
  return this.gameManagerSession_.getLastUsedPlayerId();
};
...
chrome.cast.games.GameManagerClient.prototype.onGameManagerMessage_ = function(gameManagerResponse) {
  ...
  if (!this.isInitialized_() && !gameManagerResponse.gameManagerConfig) {
    return;
  }
  if (gameManagerResponse.gameManagerConfig) {
    this.gameManagerConfig_ = gameManagerResponse.gameManagerConfig;
  }
  if (!this.isInitialized_()) {
    return;
  }
  this.previousState_ = this.currentState_;
  for (var players = [], i = 0;i < gameManagerResponse.players.length;i++) {
    var playerFromMessage = gameManagerResponse.players[i], playerId = playerFromMessage.playerId;
    players.push(new chrome.cast.games.PlayerInfo(playerId, playerFromMessage.playerState, playerFromMessage.playerData, this.gameManagerSession_.isPlayerControllable(playerId)));
  }
  this.currentState_ = new chrome.cast.games.GameManagerState(this.gameManagerConfig_.applicationName, this.gameManagerConfig_.maxPlayers, gameManagerResponse.lobbyState, gameManagerResponse.gameplayState, gameManagerResponse.gameData, gameManagerResponse.gameStatusText, players);
  var player = this.currentState_.getPlayer(gameManagerResponse.playerId);
  if (player != null && player.isControllable_ && gameManagerResponse.type == chrome.cast.games.internal.GameManagerMessageType.GAME_MESSAGE) {
    this.cachedGameMessagePlayerRecipient_ = gameManagerResponse.playerId;
    this.cachedGameMessage_ = gameManagerResponse.extraMessageData;
  }
};
chrome.cast.games.GameManagerClient.prototype.triggerEvents_ = function() {
  if (this.previousState_ != null && !this.currentState_.equals(this.previousState_)) {
    this.dispatchStateChanged_(this.currentState_, this.previousState_);
  }
  if (this.cachedGameMessagePlayerRecipient_ != null && this.cachedGameMessage_ != null) {
    this.dispatchGameMessageReceived_(this.cachedGameMessagePlayerRecipient_, this.cachedGameMessage_);
  }
  this.cachedGameMessage_ = this.cachedGameMessagePlayerRecipient_ = this.previousState_ = null;
};
chrome.cast.games.GameManagerClient.prototype.isInitialized_ = function() {
  return this.gameManagerConfig_ != null;
};
chrome.cast.games.GameManagerClient.prototype.dispatchStateChanged_ = function(currentState, previousState) {
  this.eventTarget_.dispatchEvent(new chrome.cast.games.GameManagerStateChangedEvent(currentState, previousState));
};
chrome.cast.games.GameManagerClient.prototype.dispatchGameMessageReceived_ = function(playerId, extraMessageData) {
  this.eventTarget_.dispatchEvent(new chrome.cast.games.GameManagerGameMessageReceivedEvent(playerId, extraMessageData));
};
chrome.cast.games.GameManagerClient.prototype.checkState_ = function() {
  if (!this.isInitialized_()) {
