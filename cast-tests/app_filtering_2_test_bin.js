...
function setUpPage() {
  asyncTestCase.setTests([new goog.testing.TestCase.Test("testLoadAPIScript", testLoadAPIScript), new goog.testing.TestCase.Test("testInvalidApp", testInvalidApp)]);
}
...
function testInvalidApp() {
  ...
  controller.initialize("INVALID-APP-ID", null, function(availability) {
    assertEquals(availability, chrome.cast.ReceiverAvailability.UNAVAILABLE);
    null == timerId && (timerId = setTimeout(function() {
